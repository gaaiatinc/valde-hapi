...
var dbURL = "mongodb://" + appConfig.get("app:database:login_id") +
        ...
        collection.insertOne(
            ...
            function (err, results) {
                if (err) {
                    ...
                    return resolve({results: results, new_document_id: doc._id});
                }
            });
    });
};
 ...
 * @returns {*}
 */
dbMgr.updateOne = function (collectionName, match, updateSpecs) {

    return Q.Promise(function (resolve, reject) {
        var collection = dbInstance.collection(collectionName);

        collection.updateOne(match, updateSpecs, {upsert: true},
            function (err, results) {
                if (err) {
                    logger.error(err);
                    return reject(err);
