...
let logger = logger_factory.getLogger("ConfigLRUCache", (app_config.get("env:production") || app_config.get("env:sandbox")) ? "WARN" : "DEBUG");
...
function createLocaleConfigCacheEntry(localeID) {
    ...
    return Q.Promise((resolve, reject) => {
        ...
        Q.nfcall(
                fs.readFile, path.join(configFilePathPart, "/config.json"), {
                    encoding: "utf8"
                })
            .then((localeConfig) => {
...
function getGlobalConfig() {
    ...
    return Q.Promise((resolve, reject) => {
        ...
        if (!globalConfig) {
            ...
            Q.nfcall(
                    fs.readFile, path.resolve(configFilePath), {
                        encoding: "utf8"
                    })
