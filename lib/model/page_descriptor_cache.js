...
let logger = logger_factory.getLogger("PageDescriptorLRUCache", (app_config.get("env:production") || app_config.get("env:sandbox")) ? "WARN" : "DEBUG");
let pageDescriptorCache = LRUCacheFactory(app_config.get("app:content:max_cache_size"), evictionNotifyCallback);
...
function createCacheEntry(pageID) {
    ...
    return Q.Promise((resolve, reject) => {
        ...
        Q.nfcall(
                fs.readFile, path.join(pageFilePathPart, "/metadata.json"), {
                    encoding: "utf8"
                })
            .then((metadata_str) => {
                ...
                let pageMetadataObj = JSON.parse(metadata_str);
