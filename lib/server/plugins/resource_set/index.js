...
const options_schema = Joi.object({enabled: Joi.boolean().default(false)}).required();
...
module.exports.register = (server, options, next) => {
    ...
    server.ext("onPreHandler", (request, reply) => {
        ...
        if (request.route.settings.plugins["resource_set"] && request.route.settings.plugins["resource_set"].enabled === true) {

            enabled_op = request.route.settings.plugins["resource_set"].enabled;
        }

        if (enabled_op) {
            model_factory.create_model_for_rest_request(request, reply).then((resource_set) => {
