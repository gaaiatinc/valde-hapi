...
module.exports.register = function (server, options, next) {
    ...
    server.ext("onPostAuth", function (request, reply) {
        ...
        if (request.auth.isAuthenticated) {
            if (request.auth.artifacts[appConfig.get("locale")]) {
                request.__valde.locale = request.auth.artifacts[appConfig.get("locale")];
            }
        } else if (request.state[locale_resolver_settings.tr_cookie]) {
