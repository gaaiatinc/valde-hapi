...
let options_schema = Joi.object({default_locale: Joi.string().required(), tr_cookie: Joi.string().default("vtid")}).required();
...
module.exports.register = (server, options, next) => {
    ...
    server.ext("onPostAuth", (request, reply) => {
        ...
        if ((request.auth) && (request.auth.isAuthenticated) && (request.auth.artifacts) && (request.auth.artifacts.locale)) {
