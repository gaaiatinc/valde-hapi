...
let options_schema = Joi.object({enabled: Joi.boolean().default(false)}).required();
...
module.exports.register = (server, options, next) => {
    ...
    server.ext("onPreHandler", (request, reply) => {
        ...
        if (request.route.settings.plugins["web_model"] && request.route.settings.plugins["web_model"].enabled === true) {

            enabled_op = request.route.settings.plugins["web_model"].enabled;
        }

        if (enabled_op) {
            model_factory.create_model_for_web_request(request, reply).then((model) => {
                ...
                if (redirect_to_signin) {
                    ...
                    let redirect_uri = (app_config.get("external_server_url")
                        ? app_config.get("external_server_url")
                        : "") + request.url.href;
                    let redirect_str = app_config.get("platform:auth:cookie:on_auth_failure_redirect_to") + "?" + append_next_phrase + "=" + encodeURIComponent(redirect_uri);
