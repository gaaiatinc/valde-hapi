            ...
            model_factory.create_model_for_web_request(request, reply)
                .then((model) => {
                    ...
                    if (redirect_to_signin) {
                        let append_next_phrase = app_config.get("platform:auth:appendNext") || "redirect_uri";
                        let redirect_uri = (app_config.get("external_server_url") ? app_config.get("external_server_url") : "") + request.url.href;
                        let redirect_str = app_config.get("platform:auth:on_auth_failure_redirect_to") + "?" + encodeURIComponent(append_next_phrase + "=" + redirect_uri);
