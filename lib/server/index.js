            ...
            let session_config_schema = {
                ...
                validateFunc: (temp_module_path)
                    ? Joi
                        .func()
                        .arity(2)
                        .error(new Error("Module identified in configuration (platform:plugins:hapi_auth_cookie:session_cookie_validation_module) must export an async function: function(request, session)!"))
                    : null,
                redirectTo: Joi
                    .string()
                    .required(),
...
const init = async () => {
    return new Promise(async (resolve, reject) => {
        try {
            await register_hapi_auth_cookie_plugin();
