        ...
        return Q.Promise((resolve, reject) => {
            ...
            let session_config_schema = {
                ...
                validateFunc: (temp_module_path)
                    ? Joi.func().arity(3).error(new Error("Module identified in configuration (platform:auth:cookie:session_cookie_validation_module) must export a function: function(request, session, callback)!"))
                    : null,
...
const write_process_pid = () => {
    ...
    if (pid_file_path) {
        return Q.nfcall(fs.writeFile, pid_file_path, process.pid, {encoding: "utf8"});
    } else {
        return Q();
    }
...
const init = (next) => {
    ...
    db_mgr.init(app_config).then(register_hapi_auth_cookie_plugin).then(register_hapi_auth_basic_plugin).then(register_vision_plugin).then(register_inert_plugin).then(register_platform_plugins).then(register_app_plugins).then(configure_hapi_server).then(write_process_pid).then(() => {
        next(null, server);
    }).catch((err) => {
        logger.error(err);
        throw err;
