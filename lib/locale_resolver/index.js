...
const get_request_locale = (request) => {
    ...
    let cc_regex = /^\S\S$/;
    if (!cc_regex.exec.test(x_literal_url_cc_header.trim())) {
        x_literal_url_cc_header = "";
    }

    let accept_language_header = _get(request, ["headers", "accept-language"]);
    let acclang_regex = /^((([a-zA-Z]){1,8}(-([a-zA-Z]){1,8})*|\*)(;q=([0-9](\.[0-9]+)?))?, )*(([a-zA-Z]){1,8}(-([a-zA-Z]){1,8})*|\*)(;q=([0-9](\.[0-9]+)?))?$/;
    if (!acclang_regex.exec.test(accept_language_header.trim())) {
