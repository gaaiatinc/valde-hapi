...
const dustArtifactCache = LRUCacheFactory(app_config.get("app:content:max_cache_size"), evictionNotifyCallback);
...
const createCacheEntry = (partialID, next) => {
    ...
    const __loadCompiledDust = () => {
        if (app_config.get("run_mode") == "development") {
            ...
            return Q.Promise((resolve, reject) => {
                Q.nfcall(fs.readFile, compiledPartialFilePath, {encoding: "utf8"}).then((preCompiledSource) => {
    ...
    dustPartialCreationTasks.reduce(Q.when, Q()).then(() => {
