...
const createCacheEntry = (jsxEntryRelativePath, next) => {
    ...
    const __checkIfFullGenIsNeeded = () => {
        ...
        if ((app_config.get("env:development") && (!app_config.get("liveEmulation")))) {
            ...
            return new Promise((resolve) => {
                promisify(fs.stat)(compiledJSXFilePath)
                    .then((stats) => {
    ...
    const __updateDependencies = (fileDependencies) => {
        ...
        return new Promise((resolve, reject) => {
            if (shouldPerformFullCreation) {
                ...
                promisify(fs.writeFile)(dependenciesFilePath, JSON.stringify(componentsDependencies, null, 4), {encoding: "utf8"}).then(resolve, reject);
            } else {
                promisify(fs.readFile)(dependenciesFilePath, {encoding: "utf8"})
    ...
    const __loadCompiledJSX = () => {
        return promisify(fs.readFile)(compiledJSXFilePath, {encoding: "utf8"});
    };
    ...
    const __publishCompiledJSX = (jsCode) => {
        ...
        console.log('\n\n', jsCode, '\n\n')
        fs.writeFile('./test_page.js', jsCode, err => {
            if (err) {
              console.error(err);
            }
    ...
    const __compilePageLess = () => {
        ...
        const __loadLessFile = () => {
            let lessCode = "";
            return new Promise((resolve) => {
                promisify(fs.readFile)(lessFilePath, {encoding: "utf8"})
                    .then((lessCode) => {
        ...
        const __compileLessCode = (lessCode) => {
            return new Promise((resolve) => {
                ...
                lessCompiler.render(lessCode, {
                    ...
                    promisify(fs.writeFile)(cssFilePath, renderOutput.css, {encoding: "utf8"})
                        .then(() => {
                            resolve();
                        }, (err) => {
                            resolve();
        ...
        if (shouldPerformFullCreation) {
            return promiseSequencer([__loadLessFile, __compileLessCode]);
        } else {
            return Promise.resolve();
        }
    };
    ...
    const __publishPageCSS = () => {
        ...
        const __getPageCSS = () => {
            ...
            return new Promise((resolve) => {
                promisify(fs.readFile)(pageCSSFilePath, {encoding: "utf8"})
                    .then(resolve)
                    .catch(() => {
                        resolve("");
                    });
        ...
        const __innerPublishPageCSS = (pageCSS) => {
            ...
            if (shouldPerformFullCreation) {
                ...
                const __saveResourceTask = () => {
                    return promisify(fs.writeFile)(pageCssTargetFileName, pageCSS, {encoding: "utf8"});
                };

                return promiseSequencer([__mkdirpTask, __saveResourceTask]);
        ...
        return promiseSequencer([__getPageCSS, __innerPublishPageCSS]);
    };
    ...
    const __publishGlobalCSS = () => {
        ...
        const __getGlobalCSS = () => {
            ...
            return new Promise((resolve) => {
                promisify(fs.readFile)(globalCSSFilePath, {encoding: "utf8"})
                    .then(resolve)
                    .catch(() => {
                        resolve("");
                    });
        ...
        const __innerPpublishGlobalCSS = (globalCSS) => {
            ...
            if (shouldPerformFullCreation) {
                ...
                let __saveResourceTask = () => {
                    return promisify(fs.writeFile)(targetGlobalCssFn, globalCSS, {encoding: "utf8"});
                };

                return promiseSequencer([__saveResourceTask]);
        ...
        return promiseSequencer([__getGlobalCSS, __innerPpublishGlobalCSS]);
    };
    ...
    return promiseSequencer(allTasks)
        .then(() => {
