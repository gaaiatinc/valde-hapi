...
const jsxArtifactCache = LRUCacheFactory(app_config.get("app:content:max_cache_size"), evictionNotifyCallback);
...
const pivotContentFolder = (newContentFolder) => {
    ...
    posixContentFolderPath = path.resolve(path.join(webComponentsFolder, app_config.get("app:content:root_dir"))).replace(/\\/g, "/");
...
const createCacheEntry = (jsxEntryRelativePath, next) => {
    ...
    const __checkIfFullGenIsNeeded = () => {
        ...
        if ((app_config.get("env:development") && (!app_config.get("liveEmulation")))) {
            ...
            return Q.Promise((resolve) => {
                Q.nfcall(fs.stat, compiledJSXFilePath).then((stats) => {
    ...
    const __updateDependencies = (fileDependencies) => {
        ...
        return Q.Promise((resolve, reject) => {
            if (shouldPerformFullCreation) {
                ...
                Q.nfcall(fs.readFile, dependenciesFilePath, {encoding: "utf8"}).then((fileDepsArrayStr) => {
    ...
    const __publishCompiledJSX = (jsCode) => {
        ...
        jsxCacheEntry.resourceHashFileNameDescriptor = path.posix.join(app_config.get("app_root"), "/res/js/", path.posix.dirname(jsxEntryRelativePath), "page.js");
    ...
    const __compilePageLess = () => {
        ...
        const __loadLessFile = () => {
            let lessCode = "";
            return Q.Promise((resolve) => {
                Q.nfcall(fs.readFile, lessFilePath, {encoding: "utf8"}).then((lessCode) => {
        ...
        const __compileLessCode = (lessCode) => {
            return Q.Promise((resolve) => {
                ...
                lessCompiler.render(lessCode, {
                    ...
                    Q.nfcall(fs.writeFile, cssFilePath, renderOutput.css, {encoding: "utf8"}).then(() => {
                        resolve();
                    }, (err) => {
                        resolve();
                        logger.warn(err);
    ...
    const __publishPageCSS = () => {
        ...
        const __getPageCSS = () => {
            ...
            return Q.Promise((resolve) => {
                Q.nfcall(fs.readFile, pageCSSFilePath, {encoding: "utf8"}).then(resolve).catch(() => {
                    resolve("");
                });
            });
        };
        ...
        const __innerPublishPageCSS = (pageCSS) => {
            ...
            jsxCacheEntry.pageCSSHashFileNameDescriptor = path.posix.join(app_config.get("app_root"), "/res/css", path.dirname(jsxEntryRelativePath), "page.css");
    ...
    const __publishGlobalCSS = () => {
        ...
        const __getGlobalCSS = () => {
            ...
            return Q.Promise((resolve) => {
                Q.nfcall(fs.readFile, globalCSSFilePath, {encoding: "utf8"}).then(resolve).catch(() => {
                    resolve("");
                });
            });
        };
        ...
        const __innerPpublishGlobalCSS = (globalCSS) => {
            ...
            jsxCacheEntry.globalCSSHashFileNameDescriptor = path.posix.join(app_config.get("app_root"), "/res/css", "global.css");
