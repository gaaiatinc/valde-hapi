...
const logger = logger_factory.getLogger("DynJSXCompiler", (app_config.get("env:production") || app_config.get("env:sandbox"))
    ? "WARN"
    : "DEBUG");
...
const uber_compilers_cache = LRUCacheFactory(12, evictionNotifyCallback);
...
const create_uber_cache_entry = (entityRelativePath, entityName, forServerSide) => {
    ...
    let webpackConfig = {
        ...
        resolveLoader: {
            root: [path.join(app_config.get("application_root_folder"), "node_modules")]
        },
            ...
            loaders: [
                {
                    ...
                    query: {
                        presets: [
                            require.resolve("babel-preset-es2015"), require.resolve("babel-preset-react")
                        ],
                        comments: false,
                        cacheDirectory: true,
        ...
        plugins: [
            ...
            new webpack.optimize.UglifyJsPlugin({comments: false, mangle: false, compress: false})
        ]
    };
    let uber_compiler = {};
...
const get_uber_compiler = (entityRelativePath, entityName, forServerSide) => {
    ...
    let stringKey = stringKeyFactory.createKey(entry_path);
    let uber_compiler = uber_compilers_cache.get(stringKey);

    if (!uber_compiler) {
        uber_compiler = create_uber_cache_entry(entityRelativePath, entityName, forServerSide);
        uber_compilers_cache.put(stringKey, uber_compiler);
    }
...
const compileJSXEntity = (entityRelativePath, entityName, forServerSide) => {
    ...
    const __actualCompileTask = () => {
        return Q.Promise((resolve, reject) => {
            ...
            uber_compiler.compiler.run((err, stats) => {
                // ...
                if (err) {
                    logger.error(err);
                    return reject(err);
                ...
                try {
                    ...
                    let bundle_code = uber_compiler.fs.readFileSync(output_bundle_path, "utf8");
