...
let logger = loggerFactory.getLogger("DynJSXCompiler", (appConfig.get("env:production") || appConfig.get("env:sandbox")) ? "WARN" : "DEBUG");

let resolverRoot = path.resolve(__dirname);

let webpack = require("webpack");

let staticResourcesRootFolder = appConfig.get("platform:static_resources_path") || appConstants.APP_DEFAULT_STATIC_RESOURCES_PATH;
...
function __compileJSXEntity(entityRelativePath, entityName, forServerSide, publicPath) {
    ...
    function __createStaticResourceFolder() {
        return appUtils.mkdirp(staticJsResourcesFolder, entityRelativePath);
    }
        ...
        return Q.Promise((resolve, reject) => {
            ...
            compiler.run((err, stats) => {
                ...
                if (err || (stats.compilation && (stats.compilation.errors.length > 0))) {
                    ...
                    resolve(stats.compilation.fileDependencies);
                }
            });
        });
    }
