...
const __getReactPage = (processingContext) => {
    return Q.Promise((resolve, reject) => {
        Q.nfcall(jsxArtifactCache.get, processingContext.context.pageViewID).then((react_page_entry) => {
...
const compile = (template, compileOpts, next) => {
    ...
    return next(null, (context, renderOpts, callback) => {
        ...
        renderTaskArray.reduce(Q.when, Q(processingContext)).then((markup) => {
...
const prepare = (config, next) => {
    if (render_react_script) {
        ...
        Q.nfcall(fs.readFile, path.join(__dirname, "render_react_script.js"), {encoding: "utf8"}).then((render_react_script_source) => {
